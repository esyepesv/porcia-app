...
export function LoginPage({ loading, error, destinations, onFind, onRequest, onVerify, onBack }: LoginPageProps) {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    <h1 style={{ margin: 0, fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, color: 'var(--ink)' }}>Inicia sesión</h1>
    <Input label="Documento de identidad o correo" value={identifier} onChange={(e) => setIdentifier(e.target.value)} />
    {!destinations ? <Button variant="primary" loading={loading} disabled={!identifier.trim()} onClick={() => onFind(identifier)}>Continuar</Button> : <>
      ...
      <Button variant="primary" loading={loading} disabled={code.join('').length !== 6} onClick={() => onVerify(identifier, code.join(''))}>Ingresar</Button>
    </>}
